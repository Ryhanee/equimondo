...
class Calendar {
    ...
    _init() {
        ...
        $('.select-calendar__js').on('change', function(e){
            this_element.refreshEvents();
        });
    }
    ...
    refreshEvents() {
        $('.cardCalendar').addClass('overlay-spinner');
        if(this.calendar.getEvents()){
            this.calendar.removeAllEvents();
        }

        let monitors = $('select[name="monitors[]"]').val();
        let discipline = $('select[name="discipline"]').val();
        let installation = $('select[name="installation"]').val();

        let start_date = this.calendar.view.activeStart;
        let end_date = this.calendar.view.activeEnd;

        let start_date_month = start_date.getMonth() < 12 ? start_date.getMonth() + 1 : 1;
        if (start_date_month < 10) {
            start_date_month = '0' + start_date_month;
        }

        let end_date_month = end_date.getMonth() < 12 ? end_date.getMonth() + 1 : 1
        if (end_date_month < 10) {
            end_date_month = '0' + end_date_month;
        }

        let start_full_date = start_date.getFullYear() + '-' + start_date_month + '-' + start_date.getDate();
        let end_full_date = end_date.getFullYear() + '-' + end_date_month + '-' + end_date.getDate();

        let $this = this;
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: '/modules/calendrier/modcalelistajax.php',
            data: {
                ajax_enabled: true,
                start: start_full_date,
                end: end_full_date,
                monitors: monitors,
            ...
            $('.cardCalendar').removeClass('overlay-spinner');
        });
    }
